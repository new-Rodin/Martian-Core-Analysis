...
def generate_wave(x, t, speed, wave_type='P'):
    """Generates displacement for P and S waves"""
    k = 2 * np.pi / wave_length  # Wave number
    omega = 2 * np.pi * frequency  # Angular frequency
    phase = k * x - omega * t
    
    if wave_type == 'P':
        return x, np.sin(phase), np.zeros_like(x)  # Longitudinal motion
    elif wave_type == 'S':
        return x, np.zeros_like(x), np.sin(phase)  # Transverse motion
...
root = tk.Tk()
root.title("P-Wave and S-Wave Simulation")
frame = tk.Frame(root)
frame.pack()
...
ax.set_ylim(-1, 1)
ax.set_zlim(-1, 1)
ax.set_title("P-Wave vs S-Wave Animation (Based on Research Paper)")
...
line_p, = ax.plot(xp, yp, zp, color='blue', linewidth=2, label="P-Wave (Longitudinal)")
line_s, = ax.plot(xs, ys, zs, color='red', linewidth=2, linestyle='dashed', label="S-Wave (Transverse)")
ax.legend()
